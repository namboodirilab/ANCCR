def align_events_to_reference(eventtime,eventindexseries,eventindex,referenceindex,window,binsize,resolution):
    ...
    # eventindexseries: first row of matlab eventlog file
    # eventindex: the index of event you want to align (e.g., lick)
    # referenceindex (e.g., cue1) --> you will get lick aligned to every cue1
...
def align_signal_to_reference(signal,timestamp,eventtime,eventindexseries,referenceindex,window,resolution,baselinewindow):
    ...
    #   - if resolution is zero, not doing Gaussian convolution
    # baselinewindow: if it is not nan, normalize signals by the baseline
    #   - if it is a vector: use same baseline window all reference times
    #   - if it is a matrix: each row is window for each reference time
    ...
    frameinterval = np.nanmean(np.diff(timestamp))
    ...
    nonan = np.sum(np.isnan(aligned_event),1)==0
    meanpsth = np.mean(aligned_event[nonan,:],0)
    sempsth = np.std(aligned_event[nonan,:],0)/np.sqrt(np.sum(nonan))
    if resolution > 0:
        meanpsth = nd.gaussian_filter1d(meanpsth, resolution)
        sempsth = nd.gaussian_filter1d(sempsth, resolution)
...
def calculate_auc(signal,signaltime,reference_times,window):
    ...
    # signal: signal (e.g., dff)
    # signaltime: timestamps of your signal
    # reference_times: time series of reference event
    # window: window for calculation of auc
    import numpy as np

    binsize = np.nanmean(np.diff(signaltime))
...
def cumulative_analysis(signal,xmaxwrtchangetrial):
    # do basic cumulative sum analysis

    # signal: e.g., cue response of each trial; this needs to be in chronological order
    # xmaxwrtchangetrial: only doing analysis in window of [-x x*xmaxwrtchangtrial] from changetrial

    import numpy as np
    from numpy.linalg import norm
    signalx = np.arange(1,len(signal)+1)/(len(signal)+1) # normalized x ticks (maximum is 1)
    cumsignal = np.cumsum(signal)/np.sum(signal) # cumulative sum of signal
    # distance of each point from the diagonal line
    d = [norm(np.cross(np.subtract([1,1],[0,0]),np.subtract([x,y],[1,1])))/norm(np.subtract([1,1],[0,0])) for x,y in zip(signalx,cumsignal)]
    # abruptness of change - max distance
    abruptness = max(d)
    # changetrial - trial where the distance is maximum
    changetrial = d.index(abruptness)

    # if xmaxwrtchangetrial is not nan, calculate abruptness again using [-x x*xmaxwrtchangtrial] from changetrial
    # please note that changetrial will not change anyways
